...
export async function quoteShipping(postalCode: string, items: ShipmentItem[]): Promise<ShippingQuote[]> {
  ...
  if (
    items.some(item => !item.id || !Number.isFinite(item.price) || item.price <= 0 || !Number.isInteger(item.quantity) || item.quantity < 1)
  )
    throw new Error('Há itens inválidos para calcular o frete.');

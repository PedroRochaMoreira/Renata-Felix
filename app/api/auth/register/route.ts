...
export async function POST(req: Request) {
  try {
    ...
    await sendEmail({ to: verification.email, subject: 'Confirme seu e-mail — Renata Felix', text: `Olá, ${verification.name}. Confirme seu e-mail: ${base}/verificar-email?token=${verification.token}`, html: `<p>Olá, ${escapeHtml(verification.name)}.</p><p>Para confirmar seu e-mail e proteger sua conta, <a href="${base}/verificar-email?token=${verification.token}">clique aqui</a>.</p><p>O link expira em 24 horas.</p>` });
    const res = NextResponse.json({ user: publicUser(user), verificationEmailSent: Boolean(process.env.RESEND_API_KEY && process.env.EMAIL_FROM) });
    ...
    return res;
  } catch (error) { return NextResponse.json({ error: error instanceof Error ? error.message : 'Não foi possível criar a conta.' }, { status: 400 }); }
}

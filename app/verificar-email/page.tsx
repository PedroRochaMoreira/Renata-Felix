...
export default function VerificarEmail() {
  ...
  useEffect(() => {
    ...
    if (!token) { setMessage('Link de confirmação inválido.'); return; }
    fetch('/api/auth/verify-email', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ token }) })
      .then(async response => {
        const data = await response.json();
        setMessage(response.ok ? 'E-mail confirmado com sucesso. Sua conta está protegida.' : data.error || 'Não foi possível confirmar o e-mail.');
      })

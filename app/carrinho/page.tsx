...
import { ShippingOption, useStore } from '@/app/store';
...
export default function Carrinho() {
  ...
  if (!hydrated || !catalogReady) return <><Header /><main id="conteudo" className="cartPage"><div className="pageHeading"><span className="eyebrow">Sua seleção</span><h1 className="serif">Sacola</h1></div><ListSkeleton label="Carregando sua sacola" /></main><Footer /></>;
  return <><Header /><main id="conteudo" className="cartPage"><div className="pageHeading"><span className="eyebrow">Sua seleção</span><h1 className="serif">Sacola</h1><p>{lines.length ? 'Revise suas escolhas antes de finalizar.' : 'Sua próxima escolha especial começa aqui.'}</p></div>{!lines.length ? <div className="empty"><p>Sua sacola está vazia.</p><Link href="/loja" className="button dark">Explorar a loja</Link></div> : <div className="cartLayout"><div className="cartContent"><div className="cartItems">{lines.map(({ line, product }) => {

...
export default function Login() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (mode === 'register' && !passwordPattern.test(password))
      return setMessage('Use ao menos 10 caracteres, com maiúscula, minúscula, número e símbolo.');
    setLoading(true);
    setMessage('');
    try {
      ...
      if (mode === 'register') {
        setMessage(
          data.verificationEmailSent
            ...
            : 'Conta criada com sucesso. Você poderá confirmar o e-mail assim que o envio automático estiver configurado pela loja.',
        );
      } else {
      ...
      <main id="conteudo" tabIndex={-1} className="loginPage">
        <section className="loginVisual">
          <div>
            ...
            <p>Salve suas escolhas, acompanhe pedidos e tenha uma experiência ainda mais pessoal.</p>
          </div>
        </section>
        <section className="loginContent">
          <span className="eyebrow">Área da cliente</span>
          <h1 className="serif">
            {user ? `Olá, ${user.name.split(' ')[0]}.` : mode === 'login' ? 'Que bom ter você de volta.' : 'Crie a sua conta.'}
          </h1>
          {user ? (
            <div className="loggedBox">
              <span className="role">
                <ShieldCheck size={15} /> {user.role === 'ADMIN' ? 'Acesso administrativo' : 'Cliente Renata Felix'}
              </span>
              <p>{user.email}</p>
              <Link className="button dark" href="/conta/pedidos">
                Minha conta
              </Link>
              <button
                className="textLink"
            ...
            <>
              ...
              <form className="loginForm" onSubmit={submit}>
                ...
                {mode === 'register' && (
                  <p className="passwordHint">
                    <Check size={13} /> 10+ caracteres, letra maiúscula, minúscula, número e símbolo.
                  </p>
                )}
              ...
              <div className="loginUtilities">
                <Link className="textLink" href="/recuperar-senha">
                  Esqueci minha senha
                </Link>
              </div>

...
export default function Login() {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    setMessage('');
    try {
      const response = await fetch(`/api/auth/${mode === 'login' ? 'login' : 'register'}`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ name: form.get('name'), email: form.get('email'), password }) });
      const data = await response.json();
      if (!response.ok) return setMessage(data.error || 'Não foi possível concluir esta ação.');
      setUser(data.user);
      setMessage(mode === 'register' ? 'Conta criada com sucesso. Seja bem-vinda.' : 'Login realizado com sucesso.');
    } catch {

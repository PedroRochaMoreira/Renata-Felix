...
async function homeSelections() {
  ...
  try {
    ...
    const chosen = ids.map(id => catalog.find(product => product.id === id)).filter((product): product is NonNullable<typeof product> => Boolean(product));
...
export default async function Home() {
  ...
  return <><Header /><main>
    ...
    <section className="section iconSection"><div className="sectionHead"><div><span className="eyebrow">A assinatura da loja</span><h2 className="serif">Ícones Renata Felix</h2></div><Link href="/loja" className="textLink">Ver a loja <ArrowRight size={13} /></Link></div><FeaturedIcons items={icons} /></section>

...
export default function Checkout() {
  ...
  async function pay(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const response = await fetch('/api/payments/mercadopago', {
        method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ items: cart.map(item => ({ id: item.id, size: item.size, color: item.color, quantity: item.qty })), shipping }),
      });
  ...
  if (!hydrated || !catalogReady || account === undefined) return <><Header /><main id="conteudo" className="checkoutPage"><div className="pageHeading"><span className="eyebrow">Checkout seguro</span><h1 className="serif">Finalizar pedido</h1></div><ListSkeleton label="Preparando seu checkout" /></main><Footer /></>;
  if (paymentStatus) {

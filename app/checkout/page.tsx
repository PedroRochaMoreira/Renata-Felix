...
export default function Checkout() {
  ...
  async function pay(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const response = await fetch('/api/payments/mercadopago', {
        method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ items: cart.map(item => ({ id: item.id, size: item.size, color: item.color, quantity: item.qty })), shipping, paymentMethod }),
      });
  ...
  return <><Header /><main className="checkoutPage"><div className="pageHeading"><span className="eyebrow"><LockKeyhole size={12} /> Checkout seguro</span><h1 className="serif">Finalizar pedido</h1><p>Você será direcionada ao Mercado Pago para concluir o pagamento com segurança.</p></div><div className="cartLayout"><form className="checkoutForm" onSubmit={pay}><h2 className="serif">Entrega</h2><div className="formGrid"><label>Nome completo<input name="name" required defaultValue={account.name} /></label><label className="wide">E-mail<input name="email" required type="email" defaultValue={account.email} /></label><label className="wide">Endereço<input name="street" required defaultValue={account.address?.street} /></label><label>Complemento<input name="complement" defaultValue={account.address?.complement} /></label><label>Cidade<input name="city" required defaultValue={account.address?.city} /></label><label>CEP<input name="postalCode" inputMode="numeric" required defaultValue={account.address?.postalCode} /></label></div><div className="checkoutShipping"><span className="eyebrow">Entrega selecionada</span>{shipping ? <p><b>{shipping.company} · {shipping.name}</b><span>{formatPrice(shipping.price)} · até {shipping.deliveryTime} dias úteis</span></p> : <p>Volte à sacola para escolher uma modalidade de entrega.</p>}</div><div className="paymentChoice"><span className="eyebrow">Forma de pagamento</span><label className={`paymentOption ${paymentMethod === 'PIX' ? 'active' : ''}`}><input type="radio" name="paymentMethod" checked={paymentMethod === 'PIX'} onChange={() => setPaymentMethod('PIX')} /><span><b>PIX · {pixDiscountLabel} de desconto</b><small>Aprovação na hora. O desconto vale para as peças, o frete não muda.</small></span><strong>{formatPrice(pixTotals.total)}</strong></label><label className={`paymentOption ${paymentMethod === 'OTHER' ? 'active' : ''}`}><input type="radio" name="paymentMethod" checked={paymentMethod === 'OTHER'} onChange={() => setPaymentMethod('OTHER')} /><span><b>Cartão ou boleto</b><small>Até 6x sem juros no cartão de crédito.</small></span><strong>{formatPrice(cardTotals.total)}</strong></label></div><button className="button dark fullButton" disabled={loading || !shipping}>{loading ? 'Redirecionando...' : paymentMethod === 'PIX' ? 'Pagar com PIX' : 'Ir para pagamento seguro'}</button>{message && <p className="notice">{message}</p>}<p className="checkoutTrust"><CheckCircle2 size={15} /> Seus dados são usados apenas para processar e acompanhar o pedido.</p></form><aside className="summary"><span className="eyebrow">Seu pedido</span>{lines.map(({ line, product }) => <p key={`${line.id}-${line.size}-${line.color || 'default'}`}><span>{product.name} · {line.color || defaultProductColor(product)} · {line.size} × {line.qty}</span><span>{formatPrice(product.price * line.qty)}</span></p>)}<hr /><p><span>Subtotal</span><span>{formatPrice(subtotal)}</span></p>{totals.discount > 0 && <p className="summaryDiscount"><span>Desconto PIX ({pixDiscountLabel})</span><span>− {formatPrice(totals.discount)}</span></p>}<p><span>Entrega</span><span>{shipping ? formatPrice(shipping.price) : 'A calcular'}</span></p><p className="summaryTotal"><b>Total</b><b>{formatPrice(total)}</b></p></aside></div></main><Footer /></>;
}

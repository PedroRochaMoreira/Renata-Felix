...
export default async function Loja({ searchParams }: { searchParams: Promise<{ categoria?: string; filtro?: string }> }) {
  ...
  const items =
    search.filtro === 'novidades'
      ? catalog.filter(product => product.isNew)
      : search.categoria
        ? catalog.filter(product => product.cat === search.categoria)
        : catalog;
      ...
      <main className="catalog">
        ...
        <div className="catalogBar">
          <nav className="filters" aria-label="Filtros de categoria">
            <Link href="/loja" className={!search.categoria && search.filtro !== 'novidades' ? 'active' : ''}>
              Todos
            </Link>
            <Link href="/loja?filtro=novidades" className={search.filtro === 'novidades' ? 'active' : ''}>
              Novidades
            </Link>

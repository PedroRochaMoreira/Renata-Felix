...
export default function Pedidos() {
  ...
  useEffect(() => {
    ...
    fetch('/api/account')
      ...
      .catch(reason => {
        if (active) {
          setError(reason instanceof Error ? reason.message : 'Não foi possível carregar seus pedidos.');
          setOrders([]);
        }
      });
    return () => {
  ...
  return (
    <>
      <h2 className="serif">Seus pedidos</h2>
      <p className="info">Acompanhe cada etapa da sua compra por aqui.</p>
      ...
      {orders.length ? (
          ...
          {orders.map(order => {
            ...
            return (
              <article key={order.id}>
                <div>
                  <span className="eyebrow">Pedido #{order.id.slice(-6).toUpperCase()}</span>
                  <b>
                    <CalendarDays size={13} /> {date(order.createdAt)}
                  </b>
                </div>
                <span className={`orderStatus ${order.status.toLowerCase()}`}>
                  <Icon size={12} /> {labels[order.status]}
                </span>
                <p>
                  {order.items
                  ...
                  {order.shipping ? (
                    <>
                      ...
                      <span>
                        ...
                        {order.shipping.deliveryTime ? ` · até ${order.shipping.deliveryTime} dias úteis` : ''}
                      </span>
                    </>
                  ) : (
                    <span>Entrega será confirmada após o pagamento.</span>
                  )}
                </div>
                <div className="orderTotalLine">
                  <strong>{formatPrice(order.total)}</strong>
                  {order.paymentMethod === 'PIX' && (
                    <small className="orderPixTag">
                      Pago no PIX{order.discount ? ` · economia de ${formatPrice(order.discount)}` : ''}
                    </small>
                  )}
                </div>
              </article>
        ...
        <div className="empty accountEmpty">

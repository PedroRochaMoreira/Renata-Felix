...
export default function Favoritos() {
  ...
  return <><Header /><main className="catalog"><div className="catalogTitle"><span className="eyebrow">Suas escolhas</span><h1 className="serif">Favoritos</h1><p>Peças salvas para você revisitar quando quiser.</p></div>{!hydrated ? <p className="info">Carregando seus favoritos...</p> : items.length ? <div className="grid">{items.map(product => <ProductCard key={product.id} product={product} />)}</div> : <div className="empty"><p>Você ainda não salvou peças.</p><Link className="button dark" href="/loja">Explorar a loja</Link></div>}</main><Footer /></>;
}
